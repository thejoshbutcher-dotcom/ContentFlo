...
- The boards under "Pipeline" are PROFILE DATA (`profile.pipelines`), not a fixed list: rename them, add new ones ("+" in the sidebar / mobile tab row), and per pipeline rename / recolour / add / delete / drag-reorder the steps (top-bar **Customize**, `PipelineEditor.tsx`). Views are generated by `viewDefs(pipelines)`; a pipeline's board id is `board-<pipeline id>`
...
- The LAST step of a pipeline is its finish line (done = leaves Content Buckets, never overdue). Custom steps inherit their card tab (Plan/Script/Post) from the nearest earlier step
